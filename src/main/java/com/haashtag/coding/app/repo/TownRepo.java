...
public interface TownRepo extends JpaRepository<Town, Integer> {
	...
	public Town findTownByCodesandName(@Param("districtCode") Integer districtCode,
	...
	public List<Town> findTownByCodes(@Param("districtCode") Integer districtCode,
			@Param("stateCode") Integer stateCode);

	@Query("select s from Town s where s.town like %:town%")
